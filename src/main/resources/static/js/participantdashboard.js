$(document).ready(function() {
    ...
    if ($permListParticipant.length !== 0) {
        $.getJSON("/api/v1/participant", function (data) {
            ...
            $.each(data, function() {
                messages.push(
                    "<tr><td>"+this.id+"</td><td>" + this.firstName + "</td><td>" + this.lastName + "</td><td>" +
                    "<button class='btn btn-light btn-edit' data-participant-id='" + this.id + "' " + ($permEditParticipant.length === 0 ? "disabled" : "") + ">Редактировать</button>" +
                    "<button class='btn btn-danger btn-delete' data-participant-id='" + this.id + "' " + ($permDeleteParticipant.length === 0 ? "disabled" : "") + ">Удалить</button>" +
                    "</td></tr>");
            });
