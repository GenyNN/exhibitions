        ...
        $.getJSON("/api/v1/asset", function (data) {
            ...
            $.each(data, function() {
                messages.push(
                    ...
                    "<button class='btn btn-light btn-edit' data-asset-id='" + this.id + "' " + ($permEditAsset.length === 0 ? "disabled" : "") + ">Редактировать</button>" +
                    "<button class='btn btn-danger btn-delete' data-asset-id='" + this.id + "' " + ($permDeleteAsset.length === 0 ? "disabled" : "") + ">Удалить</button>" +
                    "</td></tr>");
            });
