        ...
        $.getJSON("/api/v1/staff", function (data) {
            ...
            $.each(data, function () {
                messages.push(
                    ...
                    "<button class='btn btn-light btn-edit' data-staff-id='" + this.id + "' " + ($permEditStaff.length === 0 ? "disabled" : "") + ">Редактировать</button>" +
                    "<button class='btn btn-danger btn-delete' data-staff-id='" + this.id + "' " + ($permDeleteStaff.length === 0 ? "disabled" : "") + ">Удалить</button>" +
                    "</td></tr>");
            });
