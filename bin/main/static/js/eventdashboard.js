        ...
        $.getJSON("/api/v1/event", function (data) {
            ...
            $.each(data, function() {
                messages.push(
                    ...
                    "<td>" +
                    "<button class='btn btn-light btn-edit' data-event-id='" + this.id + "' " + ($permEditEvent.length === 0 ? "disabled" : "") + ">Редактировать</button>" +
                    "<button class='btn btn-danger btn-delete' data-event-id='" + this.id + "' " + ($permDeleteEvent.length === 0 ? "disabled" : "") + ">Удалить</button>" +
                    "</td></tr>");
            });
